...
/*
	参与者：
	AbstractFactory（抽象工厂）（抽象）：抽象工厂里面指定创建哪些抽象产品
	ConcreteFactory（具体工厂）（具体）：具体工厂实现创建抽象产品
	AbstractProduct（抽象产品）（抽象）：抽象产品指定了具有某些操作的产品
	ConcreteProduct（具体产品）（具体）：具体产品就是实现了抽象产品指定操作的具体产品

	创建对象的特点：
	可以创建多种不同的对象，但是对象的操作（方法）是一样的

	具体流程：
	1、创建一个符合抽象工厂类型的具体工厂
	2、用该工厂来创建一个符合抽象产品类型的具体产品
*/
